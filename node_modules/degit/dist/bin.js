import { a as __commonJSMin, i as require_source, n as base, o as __require, r as tryRequire, s as __toESM, t as degit } from "./src-DD58cYPF.js";
...
var require_ansi_colors = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	const create = () => {
		...
		const style = (input, stack) => {
			...
			let nl = str.includes("\n");
			let n = stack.length;
			if (n > 0 && stack.includes("unstyle")) stack = [...new Set(["unstyle", ...stack])].reverse();
		...
		colors.symbols = require_symbols$1();
...
//#endregion
//#region node_modules/enquirer/lib/utils.js
var require_utils = /* @__PURE__ */ __commonJSMin(((exports) => {
	const toString = Object.prototype.toString;
	const colors = require_ansi_colors();
	let called = false;
	let fns = [];
	const complements = {
		"yellow": "blue",
	...
	exports.unmute = (color) => {
		...
		if (name) return colors[name];
		if (color.stack.find((n) => n.slice(2) === "bg")) return colors[name.slice(2)];
		return (str) => str;
	};
	...
	exports.inverse = (color) => {
		...
		if (bg) return colors[bg.slice(2).toLowerCase()] || color;
		return colors.none;
	};
	exports.complement = (color) => {
		...
		let name = color.stack.find((n) => colors.keys.color.includes(n));
		let bg = color.stack.find((n) => n.slice(0, 2) === "bg");
		if (name && !bg) return colors[complements[name] || name];
		if (bg) {
			let comp = complements[bg.slice(2).toLowerCase()];
	...
	*/
	...
	exports.mixin = (target, b) => {
		...
		for (let key of Object.keys(b)) {
			...
			if (desc.hasOwnProperty("value")) if (target.hasOwnProperty(key) && isObject(desc.value)) if (isObject(Object.getOwnPropertyDescriptor(target, key).value)) target[key] = exports.merge({}, target[key], b[key]);
	...
	exports.mixinEmitter = (obj, emitter) => {
		...
		for (let key of Object.keys(proto)) {
			let val = proto[key];
			if (typeof val === "function") exports.define(obj, key, val.bind(emitter));
			else exports.define(obj, key, val);
		}
	};
...
//#endregion
//#region node_modules/enquirer/lib/combos.js
var require_combos = /* @__PURE__ */ __commonJSMin(((exports) => {
	/**
...
//#endregion
//#region node_modules/enquirer/lib/keypress.js
var require_keypress = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const readline = __require("readline");
	const combos = require_combos();
	...
	const keypress = (s = "", event = {}) => {
		...
		if (Buffer.isBuffer(s)) if (s[0] > 127 && s[1] === void 0) {
			s[0] -= 128;
			s = "\x1B" + String(s);
		} else s = String(s);
...
//#endregion
//#region node_modules/enquirer/lib/timer.js
var require_timer = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/enquirer/lib/state.js
var require_state = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const { define, width } = require_utils();
	var State = class {
		constructor(prompt) {
...
//#endregion
//#region node_modules/enquirer/lib/styles.js
var require_styles = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	styles.merge = (options = {}) => {
		if (options.styles && typeof options.styles.enabled === "boolean") colors.enabled = options.styles.enabled;
		if (options.styles && typeof options.styles.visible === "boolean") colors.visible = options.styles.visible;
		let result = utils.merge({}, styles, options.styles);
		delete result.merge;
		for (let key of Object.keys(colors)) if (!result.hasOwnProperty(key)) Reflect.defineProperty(result, key, { get: () => colors[key] });
		for (let key of Object.keys(colors.styles)) if (!result.hasOwnProperty(key)) Reflect.defineProperty(result, key, { get: () => colors[key] });
		return result;
	};
	module.exports = styles;
}));
//#endregion
//#region node_modules/enquirer/lib/symbols.js
...
//#endregion
//#region node_modules/enquirer/lib/theme.js
var require_theme = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/enquirer/lib/ansi.js
var require_ansi = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	const cursor = ansi.cursor = {
		...
		restore(state = {}) {
			...
			if (size) {
				...
				if (diff > 0) codes += ansi.cursor.left(diff);
				return codes;
			}
			if (value || after) {
				let pos = !input && !!initial ? -initial.length : -input.length + cursor;
				if (after) pos -= after.length;
				if (input === "" && initial && !prompt.includes(initial)) pos += initial.length;
	...
	const erase = ansi.erase = {
		...
		lines(n) {
			let str = "";
			for (let i = 0; i < n; i++) str += ansi.erase.line + (i < n - 1 ? ansi.cursor.up(1) : "");
...
//#endregion
//#region node_modules/enquirer/lib/prompt.js
var require_prompt = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const Events$1 = __require("events");
	const colors = require_ansi_colors();
	const keypress = require_keypress();
	const timer = require_timer();
	const State = require_state();
	const theme = require_theme();
	const utils = require_utils();
	...
	*/
	var Prompt = class Prompt extends Events$1 {
		constructor(options = {}) {
			super();
		...
		alert() {
			...
			else this.stdout.write(ansi.code.beep);
		}
		cursorHide() {
		...
		restore() {
			...
			let state = {
				...
				input,
				prompt,
				size: this.state.size = rest.length,
				value
			};
		...
		sections() {
			...
			let lines = buf.slice(idx).split("\n");
			let first = lines[0];
			let last = lines[lines.length - 1];
			let len = (prompt + (input ? " " + input : "")).length;
			return {
				header,
				prompt: first,
				after: len < first.length ? first.slice(len + 1) : "",
				rest: lines.slice(1),
				last
			};
		}
		...
		async close() {
			...
			try {
				...
				if (sections.rest) this.write(ansi.cursor.down(sections.rest.length));
				this.write("\n".repeat(lines));
			} catch (err) {}
			this.emit("close");
		}
		...
		async element(name, choice, i) {
			...
			state.timer = timers && timers[name];
		...
		async prefix() {
			...
			if (!utils.hasColor(element)) return (this.styles[state.status] || this.styles.pending)(element);
			return element;
		}
		async message() {
			let message = await this.element("message");
			if (!utils.hasColor(message)) return this.styles.strong(message);
			return message;
		}
		async separator() {
			...
			if (!utils.hasColor(ele)) return this.styles.muted(ele);
			return ele;
		}
		...
		async hint() {
			if (this.state.status === "pending" && !this.isValue(this.state.input)) {
				...
				if (!utils.hasColor(hint)) return this.styles.muted(hint);
				return hint;
			}
		}
		error(err) {
...
//#endregion
//#region node_modules/enquirer/lib/roles.js
var require_roles = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const utils = require_utils();
	const roles = {
...
var require_array = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var ArrayPrompt = class extends Prompt {
		...
		async toChoice(ele, i, parent) {
			...
			ele = roles(ele.role, this.options)(this, ele);
			...
			if (!this.isDisabled(ele)) this.longest = Math.max(this.longest, colors.unstyle(ele.message).length);
			let choice = { ...ele };
		...
		isDisabled(choice = this.focused) {
			if (choice && [
				...
				"completing",
				"readonly"
			].some((key) => choice[key] === true)) return true;
		...
		filter(value, prop) {
			...
			let result = (this.options.multiple ? this.state._choices : this.choices).filter(fn);
		...
		async submit() {
			...
			if (Array.isArray(value) && reorder !== false && sort !== true) value = utils.reorder(value);
	...
	function reset(prompt, choices) {
		...
		for (let choice of choices) {
			if (Array.isArray(choice.choices)) choice.enabled = choice.choices.filter((ch) => !prompt.isDisabled(ch)).every((ch) => ch.enabled === true);
...
//#endregion
//#region node_modules/enquirer/lib/prompts/select.js
var require_select = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const ArrayPrompt = require_array();
	const utils = require_utils();
	var SelectPrompt = class extends ArrayPrompt {
		...
		choiceMessage(choice, i) {
			...
			if (choice.role === "heading" && !utils.hasColor(message)) message = this.styles.strong(message);
		...
		async renderChoice(choice, i) {
			...
			if (choice.disabled) {
				if (!utils.hasColor(msg)) msg = this.styles.disabled(msg);
				return line();
			}
...
//#endregion
//#region node_modules/enquirer/lib/prompts/autocomplete.js
var require_autocomplete = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	const highlight = (input, color) => {
		...
		return (str) => {
			let i = str.toLowerCase().indexOf(val);
...
//#endregion
//#region node_modules/enquirer/lib/placeholder.js
...
//#endregion
//#region node_modules/enquirer/lib/prompts/form.js
var require_form = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const colors = require_ansi_colors();
	const SelectPrompt = require_select();
	const placeholder = require_placeholder();
	var FormPrompt = class extends SelectPrompt {
		...
		deleteForward() {
			...
			let { cursor, input } = choice;
			if (input[cursor] === void 0) return this.alert();
			choice.value = choice.input = `${input}`.slice(0, cursor) + `${input}`.slice(cursor + 1);
			return this.render();
		}
		right() {
			let choice = this.focused;
...
//#endregion
//#region node_modules/enquirer/lib/types/auth.js
var require_auth = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const FormPrompt = require_form();
...
//#endregion
//#region node_modules/enquirer/lib/prompts/basicauth.js
var require_basicauth = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const AuthPrompt = require_auth();
	...
	const factory = (authenticate = defaultAuthenticate) => {
		const choices = [{
			...
			format(input) {
				...
				return (this.state.submitted ? this.styles.primary : this.styles.muted)(this.symbols.asterisk.repeat(input.length));
			}
		}];
...
//#endregion
//#region node_modules/enquirer/lib/types/boolean.js
var require_boolean = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const Prompt = require_prompt();
	const { isPrimitive, hasColor } = require_utils();
	var BooleanPrompt = class extends Prompt {
		...
		async hint() {
			if (this.state.status === "pending") {
				let hint = await this.element("hint");
				if (!hasColor(hint)) return this.styles.muted(hint);
				return hint;
			}
		}
		async render() {
			...
			let promptLine = [
				prefix,
				await this.message(),
				this.styles.muted(this.default),
				sep
			].filter(Boolean).join(" ");
...
//#endregion
//#region node_modules/enquirer/lib/prompts/confirm.js
var require_confirm = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const BooleanPrompt = require_boolean();
...
//#endregion
//#region node_modules/enquirer/lib/prompts/editable.js
var require_editable = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const Select = require_select();
	const form = require_form().prototype;
...
//#endregion
//#region node_modules/enquirer/lib/types/string.js
var require_string = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const Prompt = require_prompt();
	const placeholder = require_placeholder();
	const { isPrimitive } = require_utils();
	var StringPrompt = class extends Prompt {
		...
		async format(input = this.value) {
			let initial = await this.resolve(this.initial, this.state);
			if (!this.state.submitted) return placeholder(this, {
				input,
				initial,
				pos: this.cursor
			});
...
//#endregion
//#region node_modules/enquirer/lib/completer.js
var require_completer = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/enquirer/lib/prompts/input.js
var require_input = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const Prompt = require_string();
	const completer = require_completer();
...
//#endregion
//#region node_modules/enquirer/lib/prompts/invisible.js
var require_invisible = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const StringPrompt = require_string();
...
//#endregion
//#region node_modules/enquirer/lib/prompts/list.js
...
//#endregion
//#region node_modules/enquirer/lib/prompts/multiselect.js
...
//#endregion
//#region node_modules/enquirer/lib/types/number.js
...
//#endregion
//#region node_modules/enquirer/lib/prompts/numeral.js
var require_numeral = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = require_number();
}));
//#endregion
//#region node_modules/enquirer/lib/prompts/password.js
var require_password = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const StringPrompt = require_string();
...
//#endregion
//#region node_modules/enquirer/lib/prompts/scale.js
var require_scale = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const colors = require_ansi_colors();
	const ArrayPrompt = require_array();
	const utils = require_utils();
	var LikertScale = class extends ArrayPrompt {
		...
		tableize() {
			...
			for (let ch of this.choices) {
				...
				ch.scale = [];
				for (let i = 0; i < this.scale.length; i++) ch.scale.push({ index: i });
			}
		...
		format() {
			if (this.state.submitted) return this.choices.map((ch) => this.styles.info(ch.index)).join(", ");
			return "";
		}
		pointer() {
			return "";
		...
		*/
		renderScaleKey() {
			...
			if (this.state.submitted) return "";
			return ["", ...this.scale.map((item) => `   ${item.name} - ${item.message}`)].map((item) => this.styles.muted(item)).join("\n");
		}
		/**
		...
		*/
		renderScaleHeading(max) {
			...
			let headings = keys.map((key) => this.styles.strong(key)).join(" ".repeat(spacing));
		...
		*/
		async renderChoice(choice, i) {
			...
			let lines = utils.wordWrap(message, {
				width: this.widths[0],
				newline
...
//#endregion
//#region node_modules/enquirer/lib/interpolate.js
var require_interpolate = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const colors = require_ansi_colors();
	const clean = (str = "") => {
	...
	*/
	...
	const tokenize = async (options = {}, defaults = {}, fn = (token) => token) => {
		...
		while (i < input.length - 1) {
			...
			if ((value === "$" || value === "#" || value === "{") && peek() === "{") {
				...
				if (defaults.hasOwnProperty(token.key)) token.initial = defaults[token.key];
				token = fn(token);
				push(token);
				keys.push(token.key);
	...
	module.exports = async (prompt) => {
		...
		let { tabstops, items, keys } = await tokenize(options, {
			...options.values,
			...options.initial
		});
		...
		return async (state = {}, submitted = false) => {
			...
			for (let token of tabstops) {
				...
				if (token.type === "template") {
					...
					let message = (item.field || {}).message || token.inner;
					if (submitted) {
					...
					if (index === state.index) if (before !== value) value = prompt.styles.underline(value);
					else value = prompt.styles.heading(colors.unstyle(value));
					index++;
				}
				if (value) state.output += value;
			}
...
//#endregion
//#region node_modules/enquirer/lib/prompts/snippet.js
var require_snippet = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const colors = require_ansi_colors();
	const interpolate = require_interpolate();
	const Prompt = require_prompt();
	var SnippetPrompt = class extends Prompt {
		...
		async submit() {
			...
			let result = colors.unstyle(output).split("\n").map((v) => v.slice(1)).join("\n");
			this.value = {
				values,
				result
			};
...
//#endregion
//#region node_modules/enquirer/lib/prompts/sort.js
var require_sort = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const hint = "(Use <shift>+<up/down> to sort)";
	const Prompt = require_select();
...
//#endregion
//#region node_modules/enquirer/lib/prompts/survey.js
var require_survey = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const ArrayPrompt = require_array();
	var Survey = class extends ArrayPrompt {
		...
		format() {
			if (this.state.submitted) return this.choices.map((ch) => this.styles.info(ch.scaleIdx)).join(", ");
			return "";
		}
		async render() {
			let { submitted, size } = this.state;
	...
	function createScale(n, options = {}) {
		if (Array.isArray(options.scale)) return options.scale.map((ele) => ({ ...ele }));
		let scale = [];
...
//#endregion
//#region node_modules/enquirer/lib/prompts/text.js
var require_text = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = require_input();
}));
//#endregion
//#region node_modules/enquirer/lib/prompts/toggle.js
	...
	var TogglePrompt = class extends BooleanPrompt {
		...
		dispatch(ch = "", key) {
			switch (ch.toLowerCase()) {
				case " ": return this.toggle();
				case "1":
				case "y":
				case "t": return this.enable();
				case "0":
				case "n":
				case "f": return this.disable();
				default: return this.alert();
			}
		}
		format() {
			let active = (str) => this.styles.primary.underline(str);
			return [this.value ? this.disabled : active(this.disabled), this.value ? active(this.enabled) : this.enabled].join(this.styles.muted(" / "));
		}
		async render() {
			let { size } = this.state;
			let header = await this.header();
...
//#endregion
//#region node_modules/enquirer/lib/prompts/quiz.js
var require_quiz = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const SelectPrompt = require_select();
...
//#endregion
//#region node_modules/enquirer/lib/prompts/index.js
var require_prompts = /* @__PURE__ */ __commonJSMin(((exports) => {
	const utils = require_utils();
	const define = (key, fn) => {
...
//#endregion
//#region node_modules/enquirer/lib/types/index.js
...
//#endregion
//#region node_modules/enquirer/index.js
var require_enquirer = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	...
	for (let name of Object.keys(prompts)) {
		...
		Enquirer[key] = run;
		if (!Enquirer[name]) Reflect.defineProperty(Enquirer, name, { get: () => prompts[name] });
	}
	const exp = (name) => {
...
//#endregion
//#region node_modules/fuzzysearch/index.js
var require_fuzzysearch = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function fuzzysearch(needle, haystack) {
		...
		outer: for (var i = 0, j = 0; i < qlen; i++) {
			...
			while (j < tlen) if (haystack.charCodeAt(j++) === nch) continue outer;
			return false;
		}
		return true;
	}
...
//#endregion
//#region node_modules/mri/lib/index.js
...
//#endregion
//#region node_modules/globrex/index.js
var require_globrex = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/globalyzer/src/index.js
var require_src = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	function parent(str, { strict = false } = {}) {
		if (isWin && str.includes("/")) str = str.split("\\").join("/");
		if (/[\{\[].*[\/]*.*[\}\]]$/.test(str)) str += "/";
		str += "a";
		do
			str = path$1.dirname(str);
...
//#endregion
//#region node_modules/tiny-glob/sync.js
var require_sync = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	const fs$1 = __require("fs");
	const globrex = require_globrex();
	const globalyzer = require_src();
	const { join: join$1, resolve, relative } = __require("path");
	const isHidden = /(^|[\\\/])\.[^\\\/\.]/g;
	let CACHE = {};
	function walk(output, prefix, lexer, opts, dirname = "", level = 0) {
		...
		for (; i < len; i++) {
			fullpath = join$1(dir, file = files[i]);
			relpath = dirname ? join$1(dirname, file) : file;
			if (!dot && isHidden.test(relpath)) continue;
			isMatch = lexer.regex.test(relpath);
			if ((stats = CACHE[relpath]) === void 0) CACHE[relpath] = stats = fs$1.lstatSync(fullpath);
			if (!stats.isDirectory()) {
				isMatch && output.push(relative(opts.cwd, fullpath));
				continue;
			}
			if (rgx && !rgx.test(file)) continue;
			!filesOnly && isMatch && output.push(join$1(prefix, relpath));
	...
	*/
	module.exports = function(str, opts = {}) {
		...
		if (!glob.isGlob) try {
			...
			let dirent = fs$1.statSync(resolved);
...
var import_enquirer = /* @__PURE__ */ __toESM(require_enquirer(), 1);
var import_fuzzysearch = /* @__PURE__ */ __toESM(require_fuzzysearch(), 1);
var import_lib = /* @__PURE__ */ __toESM(require_lib(), 1);
var import_sync = /* @__PURE__ */ __toESM(require_sync(), 1);
const __dirname = path.dirname(fileURLToPath(import.meta.url));
function parseCliArgs(argv) {
	return (0, import_lib.default)(argv.slice(2), {
		alias: {
			c: "cache",
			f: "force",
			m: "mode",
...
function displayHelp() {
	const help = fs.readFileSync(path.join(__dirname, "..", "assets", "help.md"), "utf8").replaceAll(/^(\s*)#+ (.+)/gm, (match, indent, title) => indent + import_source.default.bold(title)).replaceAll(/_([^_]+)_/g, (match, value) => import_source.default.underline(value)).replaceAll(/`([^`]+)`/g, (match, value) => import_source.default.cyan(value));
	process.stdout.write(`\n${help}\n`);
}
function getInteractiveChoices() {
	const accessLookup = /* @__PURE__ */ new Map();
	(0, import_sync.default)("**/access.json", { cwd: base }).forEach((file) => {
		const [host, user, repo] = file.replaceAll("\\", "/").split("/");
	...
	return (0, import_sync.default)("**/map.json", { cwd: base }).flatMap(getChoices).sort((a, b) => (accessLookup.get(b.value) || 0) - (accessLookup.get(a.value) || 0));
}
async function promptForSource() {
	const sourcePrompt = {
		...
		name: "src",
		suggest: (input, promptChoices) => promptChoices.filter(({ value }) => (0, import_fuzzysearch.default)(input, value)),
		type: "autocomplete"
	};
	return import_enquirer.default.prompt([
		sourcePrompt,
		{
			initial: ".",
...
async function confirmOverwrite() {
	const { force } = await import_enquirer.default.prompt([{
...
async function main(argv) {
	...
	if (!src) {
		...
		if (!(!fs.existsSync(options.dest) || fs.readdirSync(options.dest).length === 0)) {
			if (!await confirmOverwrite()) {
				console.error(import_source.default.magenta("! Directory not empty — aborting"));
				return;
			}
		}
...
function run(src, dest, args) {
	...
	d.on("info", (event) => {
		console.log(import_source.default.cyan(`> ${event.message.replace("options.", "--")}`));
	});
	d.on("warn", (event) => {
		console.warn(import_source.default.magenta(`! ${event.message.replace("options.", "--")}`));
	});
	d.clone(dest).catch((error) => {
		console.error(import_source.default.red(`! ${error.message.replace("options.", "--")}`));
		process.exit(1);
	});
}
if (!process.env.VITEST) main(process.argv).catch((error) => {
	console.error(error);
	process.exit(1);
});
//#endregion
